...
//                fieldsGrouping(Constants.RIGHT_PREDICATE_BOLT,(String)map.get("RightPredicateSourceStreamIDBitSet"),new Fields(Constants.TUPLE_ID)).setNumTasks(10);
        ...
        builder.setBolt(Constants.PERMUTATION_COMPUTATION_BOLT_ID, new PermutationBolt(),2).directGrouping(Constants.LEFT_PREDICATE_BOLT, (String) map.get("LeftBatchPermutation")).  directGrouping(Constants.RIGHT_PREDICATE_BOLT, (String) map.get("RightBatchPermutation")).setNumTasks(2);
        builder.setBolt(Constants.OFFSET_AND_IE_JOIN_BOLT_ID, new IEJoinWithLinkedListUpdated("Duration","Revenue","Time","Cost"),4)
