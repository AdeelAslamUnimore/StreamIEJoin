...
public class LeftPredicateEvaluation extends BaseRichBolt {
    ...
    public LeftPredicateEvaluation(String streamRIDInsertion, String streamSIDInsertion,String streamRIDSearch,String streamSIDSearch){
    ...
    public void execute(Tuple tuple) {
        ...
        if(tuple.getSourceStreamId().equals(streamRIDSearch)) {
            ...
            int duration= tuple.getIntegerByField(Constants.TUPLE);
            if(!listForTupleFromStreamS.isEmpty()){
            for(int i=0;i<listForTupleFromStreamS.size();i++){
                if(duration<listForTupleFromStreamS.get(i).getKey()){
        ...
        if(tuple.getSourceStreamId().equals(streamSIDSearch)) {
            BitSet bitSet = new BitSet();
            int time= tuple.getIntegerByField(Constants.TUPLE);
            if(!listForTupleFromStreamR.isEmpty()){
            for(int i=0;i<listForTupleFromStreamR.size();i++){
                if(time>listForTupleFromStreamR.get(i).getKey()){
